...
- **URL**: `/`
- **Метод**: GET
- **Описание**: Главная страница приложения. Обычно на этой странице отображается краткая информация о библиотеке и доступные действия для пользователей.
...
- **Удаление пользователя**
  - **URL**: `/users/<id>/delete/`
  - **Метод**: POST
  - **Описание**: Удаляет пользователя с указанным `id` из системы. Запрос требует подтверждения удаления.
...
- **Выдача книги**
  - **URL**: `/checkouts/add/`
  - **Метод**: GET, POST
  - **Описание**: Страница для записи новой выдачи книги. GET-запрос отображает форму для ввода данных, POST-запрос отправляет данные на сервер.

- **Возврат книги**
  - **URL**: `/checkouts/<id>/return/`
  - **Метод**: POST
  - **Описание**: Обрабатывает возврат книги с указанным `id`. Запрос обновляет статус книги и возвращает её в библиотеку.
